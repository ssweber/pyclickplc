...
def read_mdb_csv(path: str | Path) -> dict[int, AddressRecord]:
    """Read an MDB-dump CSV file into AddressRecords.

    The MDB-format CSV (exported by CLICK software) has columns:
    AddrKey, MemoryType, Address, DataType, Nickname, Use, InitialValue,
    Retentive, Comment.

    Only rows with a nickname or comment are included.

    Args:
        path: Path to MDB-format CSV file.
    ...
    with open(path, newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            retentive_raw = row.get("Retentive", "").strip()
            default_retentive = DEFAULT_RETENTIVE.get(mem_type, False)
            retentive = retentive_raw in ("1",) if retentive_raw else default_retentive
